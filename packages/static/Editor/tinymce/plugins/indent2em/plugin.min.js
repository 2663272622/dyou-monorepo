tinymce.PluginManager.add('indent2em', function(editor, url) {
    ...
    var doAct = function () {
        ...
        global$1.each(blocks, function (block) {
            ...
            if( act=='add' ){
                var firstSpan = block.querySelector('span');
                if(firstSpan && firstSpan.style && firstSpan.style.fontSize){
                    var style=dom.getAttrib(firstSpan,'style');
                    if(firstSpan.style.fontSize){
                        let indent = `calc(${firstSpan.style.fontSize} * 2)`
                        var reg = new RegExp('text-indent:[\\s]*' + indent + ';', 'ig');
